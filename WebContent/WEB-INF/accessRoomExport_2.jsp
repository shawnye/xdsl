...
<script type="text/javascript">
	...
	function isReady(form){
		if(form.export_all.value=='true'){
			var s = window.confirm('真的要导出所有接入间端口信息?');
			if(!s){
				return false;
			}
		}
		...
		if(form.export_all.value=='false' && !form.accessRooms.value){
			alert('请粘贴接入间列表');
			return false;
		}
...
<form action="${ctx }/ExportAccessRoom" method="post" accept-charset="utf-8" onsubmit="return isReady(this);">
	...
	<div>
	请粘贴接入间列表（一行仅代表一个接入间，一次不要超过31个）：<br/>
...
<div>可以通过<a href="${ctx }/report?@id=4&page_limit=200" target="_blank">【AD系统机房名称表 】</a>获取本系统机房名称。</div>
<div >
