...
<fieldset>
	<legend>帐号管理<span id="countDisplay" style="color:red;"></span> </legend><%-- 修改标题 --%>
	...
	<form action="${ctx }/report?@id=${report.id}" id="reportForm" method="post" accept-charset="utf-8">
		...
		<table id="cond_table">	<%-- BEGIN 修改条件 --%>
			<tr>
				...
				<td><input type="text" name="page_limit" value="${ page_limit }" size="10" style="color:red;background:yellow;" title="请填写大于0的整数">行</td>
				<td class="label">帐号：</td>
				<td><input type="text" name="users" value="${ users }" size="10"/></td>
				<td class="label">名称（单位）：</td>
				<td><input type="text" name="branch" value="${ branch }" size="10"/></td>
				<td class="label">操作级别：</td>
				<td> 
				<select name="m_level=">
					<option value="">[所有]</option> 
			<c:forTokens items="1-管理员,2-可写,3-可导出,4-只读" delims="," var="i">
						<option value="${ fn:substring(i,0,1) }" <c:if test="${ m_level == fn:substring(i,0,1) }">selected</c:if> >${ i }</option>
			</c:forTokens> 
				</select>
				</td>
			...
			<tr>
				...
				<select name="state=">
					<option value="">[所有]</option> 
				<c:forTokens items="1-有效,0-无效" delims="," var="i">
						<option value="${ fn:substring(i,0,1)  }" <c:if test="${ state == fn:substring(i,0,1) }">selected</c:if> >${ i }</option>
				</c:forTokens> 
				</select>
				</td>
				...
				<select name="roles">
					<option value="">[所有]</option> 
				<c:forEach items="${roleTypes }" var="i">
					<option value="${ i.name }" <c:if test="${ roles == i.name  }">selected</c:if> >${ i.label }</option>
				</c:forEach> 
				</select>
				</td>
				<td class="label">最近登录IP：</td>
				<td><input type="text" name="ip" value="${ ip }" size="10"/></td>
				<td class="label">最近登录时段：</td>
				<td> 从<input type="text" name="logon_time&gt;=" id="logon_time_start" value="${ logon_time_start }" size="7"/>
				到<input type="text" name="logon_time&lt;=" id="logon_time_end" value="${ logon_time_end }" size="7"/>
				</td>
...
function isDisabled(date) {
  ...
  Calendar.setup(
    {
      inputField  : "logon_time_start",         // ID of the input field
      ifFormat    : "%Y-%m-%d",    // the date format
      button      : "logon_time_start",       // ID of the button
      disableFunc : isDisabled
    }
  );
  Calendar.setup(
    {
      inputField  : "logon_time_end",         // ID of the input field
      ifFormat    : "%Y-%m-%d",    // the date format
      button      : "logon_time_end",       // ID of the button
      disableFunc : isDisabled
    }
  );
