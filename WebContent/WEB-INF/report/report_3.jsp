...
<fieldset>
	<legend>资源快报 <span id="countDisplay" style="color:red;"></span> </legend><%-- 修改标题 --%>
	...
	<form action="${ctx }/report?@id=${report.id}" id="reportForm" method="post" accept-charset="utf-8">
		...
		<table id="cond_table">	<%-- BEGIN 修改条件 --%>
			<tr>
				...
				<td><input type="text" name="jx" id="jx" value="${ jx }" size="10" >
					<input type="checkbox" id="jx_exact" >精确查询</td></td>
		...
		<select id="type_help">
			...
			<c:forTokens items="AD,PON-AD,PON-LAN,LAN,WLAN,FTTH" delims="," var="i">
				 ...
				 <td class="label">不统计已屏蔽端口：</td>
				<td>
				<c:choose>
					<c:when test="${ not empty mask }">
					<input type="checkbox" name="mask=" id="mask" value="#" checked="checked">
					</c:when>
					<c:otherwise><input type="checkbox" name="mask=" id="mask" value="#" ></c:otherwise>
				</c:choose>
 				</td> 
			</tr>
