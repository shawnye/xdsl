...
<fieldset>
	<legend>操作日志查询(<span style="color:red;">导出时必须填写操作时间！</span>)<span id="countDisplay" style="color:red;"></span> </legend><%-- 修改标题 --%>
	...
	<form action="${ctx }/report?@id=${report.id}" id="reportForm" method="post" accept-charset="utf-8">
		...
		<table id="cond_table">	<%-- BEGIN 修改条件 --%>
			<tr>
				<td class="label" >仅显示前</td>
				<td><input type="text" name="page_limit" value="${ page_limit }" size="5" style="color:red;background:yellow;" title="请填写大于0的整数">行</td>
				<td class="label">操作类型(事件)：</td>
				<td><input type="text" name="event" value="${ event }" size="10"/></td>
			...
			<tr>
				...
				<td>从<input type="text" name="now_time&gt;=" id="now_time_start" value="${ now_time_start }" size="10"/>
				到<input type="text" name="now_time&lt;=" id="now_time_end" value="${ now_time_end }" size="10"/>
				</td>
				 <td class="label"></td>
				<td></td>
			</tr>
...
function isDisabled(date) {
  ...
  Calendar.setup(
    {
      inputField  : "now_time_start",         // ID of the input field
      ifFormat    : "%Y-%m-%d",    // the date format
      button      : "now_time_start",       // ID of the button
      disableFunc : isDisabled
    }
  );
  Calendar.setup(
    {
      inputField  : "now_time_end",         // ID of the input field
      ifFormat    : "%Y-%m-%d",    // the date format
      button      : "now_time_end",       // ID of the button
      disableFunc : isDisabled
    }
  );
