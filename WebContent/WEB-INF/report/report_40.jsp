...
<fieldset>
	<legend>用户查询（监控） <span id="countDisplay" style="color:red;"></span> </legend><%-- 修改标题 --%>
	...
	<form action="${ctx }/report?@id=${report.id}" id="reportForm" method="post" accept-charset="utf-8">
		...
		<table id="cond_table">	<%-- BEGIN 修改条件 --%>
			<tr>
				...
				<td><input type="text" name="sbh" value="${ sbh }" width="10"></td>
			 	
			</tr>
			<tr>
			<td class="label">机房：</td>
				<td><input type="text" name="jx" value="${ jx }" width="40" style="color:red;"></td>
		...
		<tr>
			...
			<td class="label">地址：</td>
				<td>
				<input type="text" name="address" value="${ address }" id="address" size="10" >
 	
				</td>
			  <td>	AD端口类型</td>
				<td>
					<input type="text" name="type" id="type" value="${ type }" width="20">
		<select id="type_help">
			<option value="">[请选择]</option>
			<option value="ad">ad</option>
			<option value="vd">vd(EPON)</option>
	...
	<div> <%-- 修改条件 END --%>
		<input type="submit" value="查询" id="search"/>&nbsp;
 		...
 		<input type="button" value="导出csv" id="exportCsv" title="请输入条件再导出, 请不要大量导出(一次少于5000条)!"/>&nbsp;
		<input type="button" value="导出Excel" id="exportExcel" title="请输入条件再导出, 请不要大量导出(一次少于5000条)!"/>&nbsp;
 	</div>
