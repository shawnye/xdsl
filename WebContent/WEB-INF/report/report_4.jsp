...
<fieldset>
	<legend>接入间资源统计 <span id="countDisplay" style="color:red;"></span> </legend><%-- 修改标题 --%>
	...
	<form action="${ctx }/report?@id=${report.id}" id="reportForm" method="post" accept-charset="utf-8">
		...
		<table id="cond_table">	<%-- BEGIN 修改条件 --%>
			<tr>
				...
				<td><textarea name="jx" rows="5" cols="20" id="jx" title="输入一行模糊搜索，输入多行精确搜索">${ jx }</textarea></td>
				<td class="label">OLT名称：</td>
				<td><textarea name="olt" rows="5" cols="20" id="olt" title="输入一行模糊搜索，输入多行精确搜索">${ olt }</textarea></td>
