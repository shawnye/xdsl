...
// 					t = $(this).text();//包含<a>子节点数据
				...
				w.document.write('<textarea rows="24" cols="30" onfocus="this.select();">' + list + '</textarea><div>复制行数(含空行)：'+ rows +'&nbsp;&nbsp;&nbsp;<input type="button" onclick="javascript:window.close();" value="关闭"/> </div>');
				w.focus();
...
<fieldset>
	<legend>端口更新历史 <span id="countDisplay" style="color:red;"></span> </legend><%-- 修改标题 --%>
	...
	<form action="${ctx }/report?@id=${report.id}" id="reportForm" method="post" accept-charset="utf-8">
		...
		<table id="cond_table">	<%-- BEGIN 修改条件 --%>
			<tr>
				...
				<td><input type="text" name="p_id" value="${ p_id }" width="10" ></td>
				<td class="label">用户名称：</td>
				<td><input type="text" name="username" value="${ username }" width="10"></td>
			 
			</tr>
			<tr>
				 
				<td class="label">旧J_ID：</td>
				<td><input type="text" name="old_j_id=" value="${ old_j_id }" width="10"></td>
				<td class="label">新J_ID：</td>
				<td><input type="text" name="new_j_id=" value="${ new_j_id }" width="10"></td>
				<td class="label">修改人：</td>
				<td><input type="text" name="changer" value="${ changer }" width="10"></td>
			</tr>
					
			<tr>
				 
				<td class="label">修改时间&gt;=：</td>
				<td><input type="text" name="change_time&gt;=" value="${ change_time_start }" style="width:100px;" id="t_xgsj_start">
				<input type="button" id="b_xgsj_start" value="选择"></td>
				<td class="label">修改时间&lt;：</td>
				<td><input type="text" name="change_time&lt;=" value="${ change_time_end }" style="width:100px;" id="t_xgsj_end">
				<input type="button" id="b_xgsj_end" value="选择"></td>
				<td class="label">备注：</td>
				<td><input type="text" name="remark" value="${ remark }" width="10"></td>
			</tr>
	...
	<thead>
		<tr>
			...
			<c:forEach items="${ report.displayedTitles }" var="t" varStatus="s">
				<td <c:if test="${s.index == report.keyIndex }">class="key_field"</c:if> >${ t }&nbsp;<a href="javascript:void(0);" id="a_cp_col_${s.count}" title="复制[${ t }]列" col_name="${ t }"><img src="${ctx}/images/copy.gif" border="0"></a> </td>
			</c:forEach>
		</tr>
	</thead>
	<tbody>
...
function isDisabled(date) {
  ...
  Calendar.setup(
    {
      inputField  : "t_xgsj_start",         // ID of the input field
      ifFormat    : "%Y-%m-%d",    // the date format
      button      : "b_xgsj_start",       // ID of the button
      disableFunc : isDisabled
    }
  );
  Calendar.setup(
    {
      inputField  : "t_xgsj_end",         // ID of the input field
      ifFormat    : "%Y-%m-%d",    // the date format
      button      : "b_xgsj_end",       // ID of the button
      disableFunc : isDisabled
    }
  );
