...
<script type="text/javascript">
	$(function(){
		...
		$('#haoma_help').click(function(){
			setValue(this.id,'haoma');
		});
		...
		$('#search').click(function(btn){
			...
			$('#userInfo').attr("action","${ctx }/NgnDk/List");
			$('#userInfo').submit();
		});
		$('#export').click(function(btn){
			...
			$('#userInfo').attr("action","${ctx }/NgnDk/Export");
...
<fieldset>
	<legend>NGN端口查询条件 <span class="emphasis">(默认是江门地区的资料)</span> </legend>
	<div>
	<form action="${ctx }/NgnDk/List" id="userInfo" method="post" accept-charset="utf-8">
		<input type="hidden" name="@pageNo" id="pageNo" value="${page.pageNumber }">
		<input type="hidden" name="@pageSize" id="pageSize" value="${page.pageSize }">
		<table id="cond_table">
			<tr>
				<td class="label">产品号码：</td>
				<td><input type="text" name="haoma" value="${ haoma }" width="30" id="haoma">
				<input type="button" id="haoma_help" value="空号" style="background: white">
				</td>
				<td class="label">机房名称：</td>
				<td><input type="text" name="jfmc" value="${ jfmc }" width="30" >
				<td class="label">是否反极：</td>
				<td><input type="text" name="jixing" value="${ jixing }" id="fj" width="20">
		<select id="fj_help">
			...
			<tr>
				...
				<td><input type="text" name="eid" value="${ eid }" width="20" >
				<td class="label">MDF横列：</td>
				<td><input type="text" name="hl" value="${ hl }" width="20" >
				<td class="label">设备名称：</td>
				<td><input type="text" name="sbmc" value="${ sbmc }" width="20" >
			</tr>
		</table>
		<div>
...
<table border="1" class="data_table">
	...
	<tbody>
		...
		<tr>
			...
			<td>${ info.eid }</td>
			<td><fmt:formatNumber value="${ info.tid }" pattern="######"/></td>
			<td>${ info.hl }</td>
			<td>${ info.jixing }</td>
			<td>${ info.haoma }</td>
			<td>${ info.sbmc }</td>
