...
/**
 * Servlet implementation class NgnDkExportServlet
 */
public class NgnDkExportServlet extends BaseServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public NgnDkExportServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String ext = "csv";
		String downloadFileName = "NGN端口信息" + DateFormatUtils.format(new Date(), "yyyyMMdd_HHmmss") + "." + ext ;
		
		File destFile = new File(WebUtils.getDownloadDir(request) + "/phone/" + downloadFileName );
...
//				request.setAttribute(searchCondition.getEchoField(name), value);//回显
		...
		try {
			ngnDkService.exportFile(destFile, exportConfig );
